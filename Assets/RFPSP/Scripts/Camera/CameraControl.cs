	...
	void Update (){
		...
		if(Time.timeScale > 0 && Time.deltaTime > 0 && Time.smoothDeltaTime > 0 && Time.time > 0.0f){//allow pausing by setting timescale to 0
			...
			if(thirdPersonActive){
				...
				tempCamAngles = Quaternion.Euler(mainCameraTransform.localEulerAngles.x - (bobAngles.x * (IronsightsComponent.perlinHeight * Mathf.Max(1.0f, IronsightsComponent.camBobPerlinAmt.x)))  + (CameraPitchAmt * Time.deltaTime * 75.0f),//camera pitch modifiers
				                                 mainCameraTransform.localEulerAngles.y + (bobAngles.y * (IronsightsComponent.perlinHeight * Mathf.Max(1.0f, IronsightsComponent.camBobPerlinAmt.y)))  + (CameraYawAmt * Time.deltaTime * 75.0f),//camera yaw modifiers
				                                 mainCameraTransform.localEulerAngles.z - (bobAngles.z * (IronsightsComponent.perlinHeight * Mathf.Max(1.0f, IronsightsComponent.camBobPerlinAmt.z)))  + (CameraRollAmt * Time.deltaTime * 75.0f)//camera roll modifiers 
