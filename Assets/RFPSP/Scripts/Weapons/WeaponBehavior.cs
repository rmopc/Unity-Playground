...
public class WeaponBehavior : MonoBehaviour {
	...
	[Range(-0.3f, 0.3f)]
	public float weaponCrouchYPosition = -0.005f;
	[Tooltip("Amount to move weapon down when walking.")]
	[Range(-0.3f, 0.3f)]
	public float weaponWalkYPosition = -0.008f;
	...
	private float yawSmoothSpeedAmt;
	...
	[Tooltip("Amount to scale Perlin Noise in bobbing per weapon for player state (walking, zoomed, crouched, prone, sprinting).")]
	public float[] perlinBobAmts;
...
//			initialYawAngle = weaponMesh.transform.localEulerAngles.y;
	...
	/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	void LateUpdate(){
		if(doWeaponRoll && Time.timeScale > 0 && Time.deltaTime > 0 && !MouseLookComponent.dzAiming){//allow pausing by setting timescale to 0
			...
			if(!FPSPlayerComponent.zoomed || zoomIsBlock){
				rollRot = Mathf.SmoothDampAngle(rollRot, ((((rollNeutralAngle * rollSmoothSpeedAmt) * deltaAmt)
				                                           ...
				                                           + (WeaponPivotComponent.animOffsetTarg.z * deltaAmt)
				                                           - ((strafeSmoothed * deltaAmt) * strafeRoll)
				                                           - ((IronsightsComponent.HorizontalBob.dampOrg * 2.5f * rollBobAmt) * deltaAmt))
				                                          )
				                                          , ref rollVel, 
			...
			if(muzzleFlash){
				weaponMesh.transform.RotateAround(muzzleFlash.position, muzzleFlash.forward, rollRot);
			}else if(shotOrigin){
			...
			if(!unarmed){
				...
				if(FPSPlayerComponent.hitPoints >= 1.0f){
					    ...
					    || FPSWalkerComponent.hideWeapon){
							if(IronsightsComponent.reloading){
								...
								if(bulletsToReload != bulletsPerClip){
									...
									//reverse Neutral animation when sprinting
									WeaponAnimationComponent["Neutral"].speed = 1.5f;
					...
					if(!FPSWalkerComponent.hideWeapon){//dont shoot if player is climbing or weapon is hidden
						...
						if(hideAnimState && !PlayerWeaponsComponent.offhandThrowActive && WeaponAnimationComponent){
							hideAnimState = false;	
						}
		   ...
		   || (IronsightsComponent.reloading && bulletsToReload != bulletsPerClip && bulletsLeft >= bulletsToReload * 2.0f && reloadEndTime + reloadTime < Time.time))
		{
			if ((canShoot && !PlayerWeaponsComponent.switching) || doReleaseFire || meleeActive){//don't allow shooting when reloading, sprinting, or switching
				...
				if(((recoveryTime + recoveryTimeAmt < Time.time || doReleaseFire || meleeActive) && (startTime + readyTimeAmt < Time.time))){
					...
					if(AudioSettings.dspTime != oldTime){
	...
	void HitObject ( RaycastHit hit, Vector3 directionArg, bool isSecondCast = false ){
		...
		//call the ApplyDamage() function in the script of the object hit TÄNNE PUUKOTIN KAIKKI NOI RIVIT JOTKA ERIKSEEN MERKITTY MITÄ LISÄSIN
		// NÄÄ PITÄÄ LISÄTÄ MYÖS EXPLOSIVEEN KOSKA AINOASTAAN ASEEN PROJEKTIILIT TEKEE LÄMÄÄ NYT
		// LISÄKSI RÄJÄHDYKSET EI TEE LÄMÄÄ PELAAJAAN
		...
		case 0://hit object
			...
			if(hit.collider.gameObject.GetComponent<BreakableObject>()){
				...
				FPSPlayerComponent.UpdateHitTime();//used for hitmarker
			}else if(hit.collider.gameObject.GetComponent<explosive>()){ //EKA RIVI JONKA LISÄSIN
				hit.collider.gameObject.GetComponent<explosive>().ApplyDamage(damageAmt); //TOKA RIVI JONKA LISÄSIN
				FPSPlayerComponent.UpdateHitTime();//used for hitmarker //VIKA RIVI JONKA LISÄSIN
			}else if(hit.collider.gameObject.GetComponent<crate>()){ //EKA RIVI JONKA LISÄSIN
				hit.collider.gameObject.GetComponent<crate>().ApplyDamage(damageAmt); //TOKA RIVI JONKA LISÄSIN
				FPSPlayerComponent.UpdateHitTime();//used for hitmarker //VIKA RIVI JONKA LISÄSIN
			}else if(hit.collider.gameObject.GetComponent<breakablelight>()){ //EKA RIVI JONKA LISÄSIN
				hit.collider.gameObject.GetComponent<breakablelight>().ApplyDamage(damageAmt); //TOKA RIVI JONKA LISÄSIN
				FPSPlayerComponent.UpdateHitTime();//used for hitmarker //VIKA RIVI JONKA LISÄSIN
