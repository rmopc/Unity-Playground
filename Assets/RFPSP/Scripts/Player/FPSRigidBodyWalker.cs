...
public class FPSRigidBodyWalker : MonoBehaviour {
	...
	[Tooltip("Angle of ground surface that player won't be allowed to move over. Affects stairs as well.")]
	[Range(0.0f, 90.0f)]
					...
					if(sprintActive){
						if(forwardSprintOnly){
							...
							if(InputComponent.moveY > 0.05f){
								inputY = Mathf.Lerp(inputY, 1.0f, Time.deltaTime * InputYLerpSpeed);
							}else if(InputComponent.moveY < -0.05f){
								inputY = Mathf.Lerp(inputY, InputComponent.moveY, Time.deltaTime * InputYLerpSpeed);
							}else{
