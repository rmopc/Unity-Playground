				...
				if(explosionDamageAmt >= 1 && hit.collider ==  hitCollider){
					...
					switch(hitCollider.gameObject.layer){
						case 13://hit object is an NPC
							...
							break;
						case 0://hit object //TÄNNE LISÄTTY NOI MUUT LOPPUUN
							...
							}else if(hitCollider.gameObject.GetComponent<crate>()){ //ELI TÄSTÄ ETEENPÄIN NOI LOPUT
								hitCollider.gameObject.GetComponent<crate>().ApplyDamage(explosionDamageAmt);
							}else if(hitCollider.gameObject.GetComponent<breakablelight>()){
								hitCollider.gameObject.GetComponent<breakablelight>().ApplyDamage(explosionDamageAmt);
							}else if(hitCollider.gameObject.GetComponent<explosive>()){
								hitCollider.gameObject.GetComponent<explosive>().ApplyDamage(explosionDamageAmt);
							}
							break;
